...
long candies(int n, vector<int> arr) {
    vector<long> leftToRightInc;
    vector<long> rightToLeftInc;
    long resSum =0;

    leftToRightInc.push_back(1);
    for(int i=1; i<arr.size(); i++){
        if(arr[i-1]<arr[i]){
            leftToRightInc.push_back(leftToRightInc[i-1]+1);
        }else{
            leftToRightInc.push_back(1);
        }
    }

    rightToLeftInc.assign(arr.begin(), arr.end());
    rightToLeftInc[rightToLeftInc.size()-1] = 1;
    for(int i=rightToLeftInc.size()-2; i>-1; i--){
        if(arr[i]>arr[i+1]){
            rightToLeftInc[i] = rightToLeftInc[i+1]+1;
        }else{
            rightToLeftInc[i] = 1;
        }
    }

    for(int i=0; i<rightToLeftInc.size(); i++){
        resSum += max(leftToRightInc[i], rightToLeftInc[i]);
    }
    return resSum;
}
...
int main(){
    ...
    int temp;
    fstream testCaseFile("C:\\Users\\salim\\Desktop\\Shared folder on home wifi\\programming area\\HackerRank\\Algorithms\\Greedy\\Candies\\testCase11.txt", ios_base::in);
